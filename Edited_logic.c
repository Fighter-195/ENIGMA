...
// Advance the rotors with correct double-stepping logic
void spin_rotors() {
    bool middle_will_step = (rotor_offsets[0] == turnovers[0]);
    bool left_will_step = (rotor_offsets[1] == turnovers[1]);

    // Always rotate rightmost rotor
    rotor_offsets[0] = (rotor_offsets[0] + 1) % ALPHABET_SIZE;

    // Handle double-stepping logic
    if (middle_will_step || left_will_step) {
...
void encrypt_message(const char *input, char *output) {
    ...
    while (input[i] != '\0') {
        if (input[i] == ' ') {
            output[i] = ' ';
            printf("Space detected. Do you want to update the plugboard? (y/n): ");
            char resp;
            scanf(" %c", &resp);
            if (tolower(resp) == 'y') {
